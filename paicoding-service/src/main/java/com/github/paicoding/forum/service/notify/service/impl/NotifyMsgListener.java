...
public class NotifyMsgListener<T> implements ApplicationListener<NotifyMsgEvent<T>> {
     ...
     * @param event
     */
    private void removeReplyNotify(NotifyMsgEvent<CommentDO> event) {
        CommentDO comment = event.getContent();
        CommentDO parent = commentReadService.queryComment(comment.getParentCommentId());
        NotifyMsgDO msg = new NotifyMsgDO().setNotifyUserId(parent.getUserId())
                .setOperateUserId(comment.getUserId())
                .setRelatedId(comment.getArticleId())
                .setType(NotifyTypeEnum.DELETE_REPLY.getType())
                .setState(NotifyStatEnum.UNREAD.getStat())
                .setMsg("用户删除回复");
        notifyMsgDao.save(msg);
    }
     ...
     * @param event
     */
    private void removeCommentNotify(NotifyMsgEvent<CommentDO> event) {
        CommentDO comment = event.getContent();
        ArticleDO article = articleReadService.queryBasicArticle(comment.getArticleId());
        ...
        NotifyMsgDO msg = new NotifyMsgDO().setNotifyUserId(article.getUserId())
                .setOperateUserId(comment.getUserId())
                .setRelatedId(comment.getArticleId())
                .setType(NotifyTypeEnum.DELETE_COMMENT.getType())
                .setState(NotifyStatEnum.UNREAD.getStat())
                .setMsg(article.getTitle() +  ": 用户删除评论");
        notifyMsgDao.save(msg);
    }
}
